class FriendsListsController < ApplicationController
  ...
  def new
    ...
    @friends_list = current_user.friends_list.build
  end
  ...
  def correct_user
    @friends_list = current_user.friends_list.find_by(id: params[:id])
    redirect_to friends_lists_path, notice: "Not Authorized To Edit This Friend" if @friends_list.nil?
  end
